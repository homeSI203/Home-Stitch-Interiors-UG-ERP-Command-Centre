...
async function completeInstallmentPlanIfNeeded(planId: string): Promise<void> {
  ...
  const paymentDocs = paymentsSnap.docs.map((d) => ({
    ...(d.data() as Record<string, unknown>),
    id: d.id,
  }));

  const sorted = [...paymentDocs].sort(
    (a, b) => tsToDate(a.createdAt).getTime() - tsToDate(b.createdAt).getTime()
  );

  const items = sorted.map((p, i) => {
    const dateStr = tsToDate(p.createdAt).toLocaleDateString("en-UG", {
      day: "2-digit", month: "short", year: "numeric",
    });
    return {
      description: `Payment ${i + 1} – ${String(p.paymentMethod ?? "cash")} (${dateStr})`,
      quantity: 1,
